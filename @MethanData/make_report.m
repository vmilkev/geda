...
%         print(f1, '-dpsc', this.fig_resl, '-append', this.figs_fname);
    ...
    function report8
        if ( iscell(num_dat) )
            fprintf(this.f_hdl,'%s\n','');
            fprintf(this.f_hdl,'%s\n','Processed sniffer data sets:');
            fprintf(this.f_hdl,'%s\n','');
%             fprintf(this.f_hdl,'%s\n','---------------------------------------------------------------------------------------------------');
%             fprintf(this.f_hdl,'%s %10s %10s %10s %10s %10s %10s\n','Signal no. |','Skew, sec |','MF output |', 'MF output std |', 'Test statistic |', 'Signal quality 1 |', 'Signal quality 2');
%             fprintf(this.f_hdl,'%s\n','---------------------------------------------------------------------------------------------------');
                        
            fprintf(this.f_hdl,'%s\n','---------------------------------------');
            fprintf(this.f_hdl,'%s %10s %10s\n','Signal no. |','Skew, sec |','Test statistic');
            fprintf(this.f_hdl,'%s\n','---------------------------------------');

            isignal = 1;
            for j = 1:size(num_dat{1},1) % loop over number of files
                for i = 1:size(num_dat{1},2)
                    if ( num_dat{2}(j,i) ~= 0.0 ) %( (num_dat{2}(j,i) ~= 0.0) && (num_dat{3}(j,i) ~= 0.0) && (num_dat{4}(j,i) ~= 0.0) )
                        %fprintf(this.f_hdl,'%10d %11d %11.3f %14.3f %14.3f %14.3f %17.7f\n',isignal,num_dat{1}(j,i),num_dat{2}(j,i),num_dat{3}(j,i),num_dat{4}(j,i),num_dat{5}(j,i),num_dat{6}(j,i) );
            ...
            %fprintf(this.f_hdl,'%s\n','');
            fprintf(this.f_hdl,'%s\n','Re-adjusted sniffer data:');
            fprintf(this.f_hdl,'%s\n','');
            fprintf(this.f_hdl,'%s\n','SKEW ESTIMATES, sec');
            fprintf(this.f_hdl,'%s\n','-------------------------------------------------------');
            fprintf(this.f_hdl,'%2s %10s %10s %10s\n','Signal no. |','Re-adjusted |','Corrected |', 'Initial');
            fprintf(this.f_hdl,'%s\n','-------------------------------------------------------');
            for j = 1:size(num_dat,1)
                ...
                fprintf(this.f_hdl,'%5d %15d %12s %15d\n',j,num_dat(j,1),is_correct,map_dat(j,1) );
            end
            fprintf(this.f_hdl,'%s\n','-------------------------------------------------------');
            fprintf(this.f_hdl,'%s\n','');
        end
