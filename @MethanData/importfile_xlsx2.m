...
lines = this.lely_param.amsrange;
workbookFile = this.lely_param.amsfile;

opts = detectImportOptions(workbookFile);
opts.DataLines = lines;

opts.SelectedVariableNames = {char(this.lely_param.id),...
                              char(this.lely_param.time(1,1)),...
                              char(this.lely_param.time(2,1))...
                              };
LelyData = readtable(workbookFile, opts);

opts.SelectedVariableNames = {char(this.lely_param.device)};
