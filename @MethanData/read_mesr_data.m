...
data_sets_last = numel(this.mesr_param.sniffile); % expected number of data sets
...
for i = 1:numel(this.mesr_param.sniffile)
    ...
    if ( isempty(tdiff) )
        ...
        if ( any( tchange < 0.0 ) )
            ...
            this.make_report("dat", "WARNING: Some Sniffer data records are not time-consecutive, in the file no.:", i);
    ...
    if ( (data_sets - data_sets_last) > 0 )
        this.make_report("dat", "---------------------------------------------------------------------------------------------------------", []);
        this.make_report("dat", strcat("WARNING! There are time gaps > ", num2str(this.signal_gap_limit), " sec present in the sniffer records. Data file no.:                  "), i);
        this.make_report("dat", "         The entire data will be divided into the number of datasets according to the discovered gaps: ", data_sets - data_sets_last );
        this.make_report("dat", "---------------------------------------------------------------------------------------------------------", []);
