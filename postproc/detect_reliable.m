function [ reliability_and_skew, optimization_data, n_clusters ] = detect_reliable( filename, use_matl_opt )

time_skew = read_skew_xlsx(filename); % reading data from .xlsx file

n_clusters = 0; % optimal number of clusters
min_clusters = 10; % abs minimum num clusters
max_clusters = max( ceil(size(time_skew,1)/1), min_clusters ); % num clusters will be tested, depending on data size
...
% NESTED FUNCTIONS:
    ...
    % ---------------------------------------------------------------------
    function s = read_skew_xlsx(workbookFile, sheetName, dataLines)
        ...
        opts.VariableNames = ["Var1", "skew"];
        opts.SelectedVariableNames = "skew";
        opts.VariableTypes = ["char", "double"];
        % Specify variable properties
        opts = setvaropts(opts, "Var1", "WhitespaceRule", "preserve");
        opts = setvaropts(opts, "Var1", "EmptyFieldRule", "auto");
        % Import the data
        April20010202112logdevice2cleardiff = readtable(workbookFile, opts, "UseExcel", false);
        for idx = 2:size(dataLines, 1)
            opts.DataRange = dataLines(idx, :);
            tb = readtable(workbookFile, opts, "UseExcel", false);
            April20010202112logdevice2cleardiff = [April20010202112logdevice2cleardiff; tb]; %#ok<AGROW>
        end
        % Convert to output type
        s = table2array(April20010202112logdevice2cleardiff);
    end
    % ---------------------------------------------------------------------
